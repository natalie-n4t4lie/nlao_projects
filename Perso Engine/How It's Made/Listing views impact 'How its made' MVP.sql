...
SELECT
  ...
  variant_b_label,
  CASE WHEN variant_b_label IN ("seller designed,digital","seller designed, pod") THEN "seller designed"
      ...
      WHEN variant_b_label IN ("not assigned","no label someone else") THEN "no label"
...
SELECT 
*,
CASE WHEN (variant_a_main_label = "seller designed" OR variant_a_main_label = "seller sourced and curated")
     AND current_frontend_handmade = 0
...
CASE WHEN (variant_b_main_label = "seller designed" OR variant_b_main_label = "seller sourced and curated")
     AND current_frontend_handmade = 0 
...
CASE WHEN va_no_impact_listing = total_listing_ct THEN "1.no impact"
     WHEN (total_listing_ct-va_no_impact_listing)/total_listing_ct > 0 AND (total_listing_ct-va_no_impact_listing)/total_listing_ct <= 0.1 THEN "2.0-10% impacted" 
     WHEN (total_listing_ct-va_no_impact_listing)/total_listing_ct > 0.1 AND (total_listing_ct-va_no_impact_listing)/total_listing_ct <= 0.5 THEN "3.10-50% impacted" 
     WHEN (total_listing_ct-va_no_impact_listing)/total_listing_ct > 0.5 THEN "4.50%+ impacted"
     ELSE "xx" END AS va_impact_flag,
CASE WHEN vb_no_impact_listing = total_listing_ct THEN "1.no impact"
     WHEN (total_listing_ct - vb_no_impact_listing)/total_listing_ct > 0 AND (total_listing_ct - vb_no_impact_listing)/total_listing_ct <= 0.1 THEN "2.0-10% impacted" 
     WHEN (total_listing_ct - vb_no_impact_listing)/total_listing_ct > 0.1 AND (total_listing_ct - vb_no_impact_listing)/total_listing_ct <= 0.5 THEN "3.10-50% impacted" 
     WHEN (total_listing_ct - vb_no_impact_listing)/total_listing_ct > 0.5 THEN "4.50%+ impacted"
     ELSE "xx" END AS vb_impact_flag,  
...
SELECT 
listing_id,
why_working_with_partner,
-- why_working_with_partner_other,
role_in_design_process,
partners_role
FROM `etsy-data-warehouse-prod.etsy_shard.shop_production_partner_listing_association` sppla
JOIN `etsy-data-warehouse-prod.etsy_shard.shop_production_partner` spp
  ON sppla.production_partner_id = spp.production_partner_id
...
SELECT
CASE WHEN why_working_with_partner IS NULL THEN "no_production_partner" ELSE why_working_with_partner END AS why_working_with_partner,
-- CASE WHEN why_working_with_partner_other IS NULL THEN "no_production_partner" ELSE why_working_with_partner_other END AS why_working_with_partner_other,
CASE WHEN role_in_design_process IS NULL THEN "no_production_partner" ELSE role_in_design_process END AS role_in_design_process,
CASE WHEN partners_role IS NULL THEN "no_production_partner" ELSE partners_role END AS partners_role,
COUNT(DISTINCT listing_id) AS listing_ct,
SUM(past_year_gms) AS past_year_gms,
COUNT(DISTINCT user_id) AS seller_ct
FROM `etsy-data-warehouse-prod.rollups.active_listing_basics`
...
CASE WHEN role_in_design_process IS NULL THEN "no_production_partner" ELSE role_in_design_process END AS role_in_design_process,
CASE WHEN partners_role IS NULL THEN "no_production_partner" ELSE partners_role END AS partners_role,
COUNT(*) AS listing_view
FROM `etsy-data-warehouse-prod.analytics.listing_views`
LEFT JOIN cte USING (listing_id)
WHERE _date >= current_date - 31 and _date <= current_date - 1
GROUP BY 1,2,3
ORDER BY 4 DESC
;
