...
-- INPUT
-------------------------------------------------------------------------------------------
DECLARE config_flag_param STRING DEFAULT "perso_engine.recs.because_you_purchased_compl_v5";
...
CREATE OR REPLACE TABLE `etsy-data-warehouse-dev.nlao.events` AS (
    ...
    FROM
        UNNEST([
            ...
            "search_click",
            "search_purchase",
            "boe_homescreen_evergreen_interests_seen"
        ]) AS event_id
);
...
WITH module_seen AS (
SELECT 
DISTINCT bucketing_id,
event_value
FROM `etsy-data-warehouse-dev.nlao.events_per_unit`
WHERE event_id = "boe_homescreen_evergreen_interests_seen"
AND event_value >= 1.0
)
, searched AS (
SELECT 
...
event_value
FROM `etsy-data-warehouse-dev.nlao.events_per_unit`
WHERE event_id = "search_page_1"
AND event_value >= 1.0
)
SELECT 
variant_id,
CASE WHEN ms.event_value IS NOT NULL THEN 1 ELSE 0 END AS module_seen,
CASE WHEN s.event_value IS NOT NULL THEN 1 ELSE 0 END AS searched,
COUNT(DISTINCT b.bucketing_id) AS browsers,
SUM(s.event_value) AS total_search
FROM `etsy-data-warehouse-dev.nlao.ab_first_bucket` b
LEFT JOIN module_seen ms
  ON b.bucketing_id = ms.bucketing_id
LEFT JOIN searched s
  ON b.bucketing_id = s.bucketing_id
GROUP BY 1,2,3
;

WITH module_tapped AS (
SELECT 
DISTINCT bucketing_id,
event_value
FROM `etsy-data-warehouse-dev.nlao.events_per_unit`
WHERE event_id = "homescreen_tapped_evergreen_listing"
AND event_value >= 1.0
)
, searched AS (
SELECT 
...
SELECT 
variant_id,
CASE WHEN mt.event_value IS NOT NULL THEN 1 ELSE 0 END AS module_tapped,
CASE WHEN s.event_value IS NOT NULL THEN 1 ELSE 0 END AS searched,
COUNT(DISTINCT b.bucketing_id) AS browsers,
SUM(s.event_value) AS total_search
FROM `etsy-data-warehouse-dev.nlao.ab_first_bucket` b
LEFT JOIN module_tapped mt
  ON b.bucketing_id = mt.bucketing_id
LEFT JOIN searched s
  ON b.bucketing_id = s.bucketing_id
GROUP BY 1,2,3
ORDER BY 1,2,3
