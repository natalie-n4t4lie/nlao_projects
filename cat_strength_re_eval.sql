...
WHERE split(full_path, '.')[safe_offset(0)] = "home_and_living" OR split(full_path, '.')[safe_offset(1)] in ("prints", "collectibles", "painting", "drawing_and_illustration")
),
hl_search AS (
SELECT
query,
...
SELECT DISTINCT
  v._date,
	...
	v.third_channel,
	v.platform,
  COALESCE(v.landing_cat_page_top_cat,t1.top_cat,q.top_cat) AS top_cat,
  COALESCE(v.landing_cat_page_top_cat,t1.second_cat,q.second_cat) AS second_cat,
  COALESCE(v.landing_cat_page_third_cat,t1.third_cat,q.third_cat) AS third_cat,
  1 AS is_hl
FROM landing_p365 v
...
OR t1.taxonomy_id IS NOT NULL -- Landing event is a listing in the home & listing category, or in key art & collectibles subcategories
...
OR
regexp_contains(lower(regexp_replace(regexp_replace(regexp_substr(landing_event_url, "featured/([^?]*)"), "-", " "), "\\%27", "")), '(\?i)\\bgift') -- Landing on an Editor’s Picks page with a title containing “gift”
)
;
