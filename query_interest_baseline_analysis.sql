...
FROM `etsy-data-warehouse-prod.rollups.query_level_metrics` l
LEFT JOIN `etsy-data-warehouse-dev.nlao.query_intent_labels_dedupped` qi 
  ON qi.query_raw = l.query
LEFT JOIN cte q 
  ON q.clean_query = l.query
GROUP BY 1,2,3,4,5
;

-- INTEREST TYPE COUNT DISTRIBUTION
  -- QUERY BIN
WITH cte AS (
SELECT
  clean_query,
...
-- POPULAR INTEREST TYPE BY THRESHOLD
  -- 0
WITH cte AS (
SELECT
distinct l.query,
...
SELECT
CASE WHEN query_count >=10 THEN 10 
  ELSE query_count 
  END AS query_count,
