...
-- INPUT
-------------------------------------------------------------------------------------------
DECLARE config_flag_param STRING DEFAULT "app_home.boe_homescreen_evergreen_interests.v4_fandoms";
...
CREATE OR REPLACE TABLE `etsy-data-warehouse-dev.nlao.ab_first_bucket` AS (
    SELECT
        ...
        DATE_DIFF(end_date,DATE(MIN(bucketing_ts)),day) AS days_in_experiment
    FROM
...
CREATE OR REPLACE TABLE `etsy-data-warehouse-dev.nlao.events_per_unit` AS (
    ...
    GROUP BY
        bucketing_id, variant_id, event_id, days_in_experiment
);
...
IF bucketing_id_type = 1 THEN -- browser data (see go/catapult-unified-enums)
    CREATE OR REPLACE TABLE `etsy-data-warehouse-dev.nlao.post_bucketing_custom_events` AS (
        ...
        SELECT
            ...
            event_value,
            ROW_NUMBER() OVER (
                PARTITION BY bucketing_id, variant_id, event_id, days_in_experiment
...
CREATE OR REPLACE TABLE `etsy-data-warehouse-dev.nlao.visits_per_unit` AS (
    ...
    GROUP BY
        bucketing_id, variant_id,days_in_experiment
);
...
GROUP BY
    event_id, variant_id, days_in_experiment
ORDER BY
    event_id, variant_id, days_in_experiment;
